...
- After a red/stripe/boot fail or a playtest “wrong picture” report: read **`build/emu.log` first**, then the framebuffer HUD, then GOTCHAS. Spawn **debug orchestrator + one Adversarial** (AGENTS.md). Do not solo-guess. Do not spawn Disc/FS + PVR + Boot as extra agents.
