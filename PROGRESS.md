...
| 14.6 | Per-frame `dir.ff` reads | source | Enter-path TOC + `FILE*`; `file_blob` charges `ff_toc_bytes` not the pack. Play-path `SEEK_SET` / skip, no `fopen`. Catalog PR still open |
