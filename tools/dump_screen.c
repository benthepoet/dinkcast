    ...
    for (i = 1; i <= 99; i++) {
        if (s.sprite[i].active) {
            printf("sprite %d seq=%d frame=%d type=%d vis=%d xy=%d,%d script=%s"
                   " warp=%d map=%d to=%d,%d parm=%d brain=%d speed=%d walk=%d"
                   " idle=%d attack=%d hit=%d\n",
                   ...
                   (int)s.sprite[i].parm_seq, (int)s.sprite[i].brain,
                   (int)s.sprite[i].speed, (int)s.sprite[i].base_walk,
                   (int)s.sprite[i].base_idle, (int)s.sprite[i].base_attack,
                   (int)s.sprite[i].base_hit);
            actives++;
        }
    }
