...
- **KOS ISO9660 / Flycast GD-ROM is not POSIX disk.** A second `fopen`/`fseek(SEEK_END)`/`fread` of a file already read (`hard.dat`, `map.dat`, walk `dir.ff`, `tiles/tsNN.bmp`) can **deadlock** the FS thread with PVR. Symptom: screen swap “loads” then freeze; serial stops mid `ff load`. Permanent rules: **never reopen `map.dat`**; **never `SEEK_END`** (8 KiB + `thd_pass`); **LRU `dir.ff` + tile sheets**; **keep editor CPU pixels when leaving a screen** (evict PVR only) so house→yard→house is not a disc read. Pinning walk/idle alone is not enough.
