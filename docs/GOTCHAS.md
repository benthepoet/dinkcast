...
- **Redream rejects the Flycast MIL-CD CHD** (`sector_fmt == GD_SECTOR_ANY || sector_fmt == track->sector_fmt`): it wants GD-ROM 2352. `make chd-redream` writes `build/dinkcast-redream.chd` (3-track GDI, MODE1/2352, ISO LBAs rebased from mkdcdisc 11702 to 45000). Do not point `make emu` at that file unless Flycast is re-checked.
