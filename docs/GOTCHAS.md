...
- **KOS ISO9660 / Flycast GD-ROM is not POSIX disk.** A second `fopen`/`fseek(SEEK_END)`/`fread` of a file already read can deadlock FS vs PVR. House door: `swap stamp ok` then silence is **ts02.bmp**. Boot: `ini_count` of seq 12–18 / 71–79 / 101–109 pulls **bottles (75)** and **bow/hit (101)**; serial stops at `ff load graphics/dink/bow/hit/dir.ff`. Count frames when the seq is first drawn. Do **not** `vid_waitvbl` after every fclose (vblank may not fire during load → hang before title/house). `thd_sleep` only between swap I/O. Never reopen `map.dat`; never `SEEK_END`; 8 KiB reads; drop unpinned `dir.ff` after decode.
