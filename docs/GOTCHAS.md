...
- **KOS ISO9660 / Flycast GD-ROM is not POSIX disk.** A second `fopen` of a file already read can deadlock. Yard uses home- frames 1/2/11 then **drops the pack**; east needs 3/4 and hangs at `ff load graphics/struct/home/dir.ff`. Decode every frame of a seq (n≤24) while the pack is open. Boot: do not `ini_count` 71–79/101–109 (seq 75 bottles, 101 bow). No `vid_waitvbl` after every fclose. Never reopen `map.dat`; never `SEEK_END`.
