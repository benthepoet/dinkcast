...
- **`create_sprite` is a live `BrainSpr`, not an editor row.** FreeDink `add_sprite_dumb` takes the first inactive `spr[]`. Dinkcast still draws type-0 from the map snap, so skip **active editor** slots (else the gate girl overlays a wall and vanishes on `brains_apply`). Screen MAIN runs **before** `brains_enter`; do not memset `created` slots. Load her `pseq` after `edraw_load_screen` (play-path `ensure_frame` will not `fopen` `graphics/people/girl/dir.ff`). Per-frame `memcpy` of the snap still needs `brains_apply` to overlay created `active`/`type`.
