...
- **Flycast playtest is CHD, not DiscJuggler CDI.** `chdman createcd -i file.cdi` reports 0 tracks and hangs at `-nan%`. mkdcdisc `-I` dumps the data-track ISO; `make chd` wraps it as a GDI (dummy 4 s audio + data LBA 11702, same as mkdcdisc `ms_block`) and compresses that. Hardware still burns the `.cdi`. Need `chdman` from `mame-tools`.
