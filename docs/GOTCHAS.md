...
- **Do not open `ts02.bmp` after a dir.ff storm.** House door: `ff ok trees` then `tiles slurp tiles/ts02.bmp` with no ok. Warm ts02/ts03 right after the first house atlas. Keep decoded sheets; do not evict and re-slurp on swap.
