...
- **Do not reopen Dink walk/idle `dir.ff` after another pack.** Walk is ~423 KB. House re-enter loads idle then walk again; the second `/cd` open of walk can hang after `swap edraw upload` / `attach=8` (house is on screen, controls dead). Pin idle+walk in `ff_cached`.
