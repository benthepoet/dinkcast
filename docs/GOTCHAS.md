...
- **KOS ISO9660 / Flycast GD-ROM is not POSIX disk.** A second `fopen`/`fseek(SEEK_END)`/`fread` of a file already read (`hard.dat`, `map.dat`, walk `dir.ff`, `tiles/tsNN.bmp`) can **deadlock** the FS thread with PVR. House door: `swap stamp ok` then silence is **ts02.bmp** after `home-`/`trees` dir.ff, not a missing warp. **never reopen `map.dat`**; **never `SEEK_END`**; 8 KiB reads; **`vid_waitvbl` after each slurp fclose**; drop unpinned `dir.ff` once frames are decoded; keep editor CPU pixels; cache tilesheets.
