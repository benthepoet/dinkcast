...
- **KOS ISO9660 / Flycast GD-ROM is not POSIX disk.** A second `fopen`/`fseek(SEEK_END)`/`fread` of a file already read (`hard.dat`, `map.dat`, walk `dir.ff`) can **deadlock** the FS thread with PVR. Symptom: screen swap “loads” then freeze; serial stops mid `ff load`. Permanent rules: **never reopen `map.dat`** (keep `FILE*`); **never `SEEK_END`** (read to EOF in 8 KiB + `thd_pass`); **LRU-cache `dir.ff`**; **reuse editor `(seq,frame)` across swaps** so neighbors are decode/upload only. Pinning one pack is not enough.
