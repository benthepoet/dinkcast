...
- **Redream rejects the Flycast MIL-CD CHD** (`sector_fmt` cooked 2048). Empty dummy as GDI track 1 at LBA 0 is the **BIOS menu** (no IP.BIN). `make chd-redream` puts the rebased ISO on **track 1** as MODE1/2352 (LBA 0) plus two tiny trailing tracks. `make emu` still uses `build/dinkcast.chd`.
