...
- **`say` is sprite-anchored, not a HUD bar.** FreeDink `say_text` then `text_brain`: each frame `x = owner.x-75`, `y = owner.y-100` (default defense) unless `text_owner==1000`. Snapshot-once leaves `FINDDUCK` “There's that duck!!” behind Dink. Wrap 150, `` `X `` color prefix. A **or** B advances `say_stop`. Do not punch on B while a line is up.
