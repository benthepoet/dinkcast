...
- **One `map.dat` screen holds several visions.** Start house: vision **0** intact, **1** fire/`explo-`, **2** holes. `&vision` starts at 0. Draw `vision==0 || vision==current`; type **2** is hardness-only. Drawing every `active` stacks the burned house on the intact one. Not a DinkC requirement to *hide* fire at new-game — only to *change* vision later.
